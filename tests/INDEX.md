...
- Structured Output の `--output-schema` ファイル生成、JSON/テキストの再試行、schema 検証、意味的検証失敗を扱います。
- `codex exec` 呼び出しログ、`subcommand_log` への通知、出力プレビュー、`INDEX.md` 事前メンテナンスと `skip_index_maintenance` を確認します。
...
- `run_codex_exec()` の引数、`read_only` / `workspace-write`、`--json`、`--output-last-message`、`--output-schema`、`reasoning_effort` の扱いを確認したいとき。
- Structured Output の parse 失敗、schema 不一致、`enum` や文字列長制約、意味的検証失敗に対するリトライとエラー表示を確認したいとき。
- Codex CLI 呼び出しログ、`subcommand_log` への tee、出力の先頭 80 文字プレビュー、quota 枯渇時の待機・疎通確認・`--resume` 再実行フローを確認したいとき。
- `INDEX.md` の事前メンテナンスや `skip_index_maintenance`、workspace-write 時の oracle 変更ガードを確認したいとき。
- `_extract_session_id`、`_resume_command`、`_prepare_codex_exec_paths`、`_TeeTextIO`、`_init_git_repo` / `_git` の補助テストを追いたいとき。

## Do not read this when

- `tests/test_indexing.py` の `INDEX.md` メンテナンス処理そのものを確認したいときは、そちらを読むべきです。
- `tests/test_subcommands.py` や `src/sub_commands` のような個別サブコマンドの業務ロジックだけを確認したいときは、この目次では範囲が広すぎます。
- `tests/test_repo.py` の git 共通処理や、`tests/test_timestamps.py` の時刻書式だけを確認したいときは、このファイルを読む必要はありません。
- `README.md`、`AGENTS.md`、`memo` の運用や編集可否だけを確認したいときは、このテスト目次ではなく別の案内を参照すべきです。

## hash

- ad80f78abd8fc110ef369326c267654b3f9119d45e315cc1c1bbadcaa6797926
...
- `gitignore` 除外、空ディレクトリ、`build` / `tmp`、バイナリ、UTF-8 文字境界、`memo`、既存 `INDEX.md` の再生成と再利用、自動コミット範囲を検証します。
...
- `gitignore` 除外、空ディレクトリ、`build` / `tmp`、バイナリ、UTF-8 境界、`memo` ディレクトリの扱いを見直したいとき。
...
- `commons.indexing.maintain_indexes` の実装そのものを追いたいとき。
...
- 999fff2bf957e8a46e2adc8119979d23120cb8de490d145c0684b3488632a844
