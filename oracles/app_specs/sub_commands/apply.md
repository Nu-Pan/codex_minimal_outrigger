...
3. 不整合調査・修正ループ（最大 N 回）
    1. Codex CLI に、`oracles` と実装との明確な不整合を調査させる
    2. 調査結果の不整合リストが空であれば、追加修正対象が検出されなかったものとしてループを終了する
    3. 調査結果の不整合リストが空でなければ、検出された不整合に対する修正作業を Codex CLI に依頼する
...
- Structured Output の schema は以下の通り
    ...
    ```
- 「不整合のリスト」上の項目は、全て要修正項目として扱う
- 「不整合のリスト」が空の場合のみ「検出された不整合なし」と扱う
