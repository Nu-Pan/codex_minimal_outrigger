...
`cmoc session join` は、現在 checkout している `<cmoc-session-branch>` を、その session metadata に記録された `<cmoc-session-home-branch>` へ merge する。
...
`cmoc session join` は `<cmoc-session-branch>` 上でのみ実行できる。

以下の場合はエラー終了する。

- 同じ session に `running` 状態の apply run が存在する
- 同じ session に `merge_pending` または `merge_conflict` 状態の apply run が存在する
- apply run metadata 上、`<cmoc-session-branch>` への統合結果が確認できない apply run が存在する
...
8. merge が完了したら session metadata の `state` を `joined` にする
