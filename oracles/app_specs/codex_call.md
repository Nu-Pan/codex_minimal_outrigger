...
- quota 枯渇の判定方法
    - Codex CLI の最終出力メッセージ `quota exhausted` が含まれる場合 quota 枯渇とみなす
- ユーザー向けメッセージについて
...
### `Selected model is at capacity` で失敗した場合

- Codex CLI の最終出力メッセージに `Selected model is at capacity` が含まれている場合
- 8 回までリトライする
