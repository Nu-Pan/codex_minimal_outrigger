...
- `src` は cmoc CLI の実装ルートで、`main.py`、`commons/`、`sub_commands/` をまとめる上位ディレクトリです。
- `main.py` は CLI のエントリーポイントで、Typer のルートと各サブコマンド群を組み立てます。
- `commons/` は共通基盤処理、`sub_commands/` は `init`、`session`、`apply`、`review` 系の本体実装を収めます。
...
- `src/commons` の共通処理だけを追いたいとき。
- `src/sub_commands` 配下の個別サブコマンド実装だけを確認したいとき。
- `main.py` の詳細な CLI 例外処理や引数定義だけを見たいとき。

## hash

- de33de9b878ad7ef89ebb6bb9c20fea6f334416f18826db72ed30bef06552f50
...
- `tests` 配下の pytest 群を機能別に案内するための目次です。
- 共通設定、git まわり、INDEX.md メンテナンス、Codex CLI ラッパー、サブコマンド、命名規則、タイムスタンプ、レポート保存のテストをまとめて辿れます。
- 各テストファイルの責務を見分けて、必要な回帰テストだけへ素早く移動するための入口です。

## Read this when

- `tests` ディレクトリ全体が何を守っているか、入口だけ先に把握したいとき。
- `conftest.py`、`test_codex.py`、`test_repo.py`、`test_indexing.py` など、個別テストファイルの役割を素早く見分けたいとき。
- pytest の共通設定、git / INDEX.md / Codex 呼び出し / サブコマンド / レポート保存まわりの回帰テスト範囲を確認したいとき。

## Do not read this when

- `src` 配下の実装コードそのものを追いたいとき。
- `oracles` の正本仕様や `<cmoc-root>/INDEX.md` など、tests 以外のルーティング文書を確認したいとき。
- 個別の CLI 仕様や共通処理の実装詳細を、テストではなく本体コードから確認したいとき。

## hash

- 546455193396bbc3b9bd49c9f658e57869d402013234c4e3e93ad9776252e9ac
