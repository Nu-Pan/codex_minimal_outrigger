...
- `src/main.py` は Typer による CLI エントリーポイントとサブコマンド登録を担当します。
- `src/commons` はサブコマンド間で共有する共通基盤処理を収めます。
- `src/sub_commands` は `cmoc` の各サブコマンド本体実装を収めます。
- この `INDEX.md` は、実装本体の参照先を切り分けるための入口です。

## Read this when

- cmoc の実装ファイルをどのディレクトリから読むべきか判断したいとき。
- CLI エントリーポイント、共通基盤、サブコマンド本体の役割分担を把握したいとき。
- `src` 配下のどこに実装を置くべきか、またはどこを修正すべきか迷ったとき。
- `src` 全体の構造をざっと確認して、個別モジュールへ進む前の入口が欲しいとき。

## Do not read this when

- `cmoc` の利用手順や各サブコマンドのユーザー向け仕様だけを知りたいとき。
- `oracles` 側の正本仕様そのものを確認したいとき。
- `README.md`、`AGENTS.md`、`memo` などの運用ルールだけを確認したいとき。
- テスト規約や開発環境ルールだけを確認したいとき。

## hash

- 5af6502894dd56a6fed5d2dc3c3807f08d5f46e4dd71a771a1282d87da7f1d05
