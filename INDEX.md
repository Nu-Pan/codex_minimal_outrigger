...
- `cmoc` の Python 実装本体をまとめる `src` ディレクトリの入口です。
- CLI のエントリーポイント `main.py`、共通基盤 `commons`、各サブコマンド実装 `sub_commands` を含みます。
- 実装の起点と再利用部品とコマンド別実装の役割分担を素早くたどるための目次です。

## Read this when

- `src` 配下でどこに CLI の入口や共通処理、サブコマンド本体があるか把握したいとき。
- `cmoc` の実装を変更するときに、まずどのモジュールへ進むべきか整理したいとき。
- `main.py`、`commons`、`sub_commands` の相互関係を俯瞰したいとき。

## Do not read this when

- `src` 配下の特定ファイルの実装詳細だけを確認したいときは、該当ファイルや子ディレクトリの `INDEX.md` を直接読むべきです。
- テストコードや `oracles` の仕様断片だけを確認したいときは、この目次を読む必要はありません。
- 実際のコード変更や挙動確認が目的で、トップレベルの構成把握が不要なとき。

## hash

- 0573386ef7e4d26f4bf62a11fc72e8ae2b7490d63cb8ff83e92e74ba4bf0e628
...
- `tests` 配下の pytest 群をまとめる入口です。
- `conftest.py` の共通設定と、`commons.codex`、`commons.repo`、`commons.indexing`、サブコマンド、タイムスタンプ、ファイル命名規則を検証するテスト群への導線を整理します。
- cmoc の決定論的な制御ロジックと git・filesystem の周辺挙動を確認するための目次です。

## Read this when

- cmoc の pytest テストが何を検証しているか全体像をつかみたいとき
- `commons.codex`、`commons.repo`、`commons.indexing`、サブコマンド、タイムスタンプ周りのテストを探したいとき
- 新しいテストを追加・修正する前に、既存のテスト配置と役割分担を確認したいとき

## Do not read this when

- cmoc の実装ロジックや CLI の本体を確認したいとき
- `src` や `oracles` の仕様だけを確認したいとき
- テストではなく利用手順や設計方針の文書を探しているとき

## hash

- 002ad1de8d2d00873704fc0555f6c53f73449e4ce66830cc1f56765e66f98204
