...
- `cmoc` の実装コードをまとめる `src` 配下の入口です。
- `main.py` は CLI の起動点、`commons/` は共通基盤、`sub_commands/` は各サブコマンド実装を担います。
- 実行基盤とサブコマンド群の関係を把握するための総覧です。

## Read this when

- `cmoc` のエントリーポイントと共通基盤の役割分担を確認したいとき。
- `commons/`、`main.py`、`sub_commands/` のいずれを修正・レビューする前に全体像をつかみたいとき。
- `src` 配下に新しい共通機能やサブコマンド実装を追加するとき。

## Do not read this when

- 個別サブコマンドの引数や業務ロジックだけを確認したいとき。
- `oracles` 配下の正本仕様や運用手順だけを確認したいとき。
- `src` 以外のディレクトリ構成やテストだけを確認したいとき。

## hash

- cc90642d5ebd57ac375d097cad41e1c99150ed628c7f21645f5cc8f1afed395b
...
- pytest 共通設定と、CLI・共通処理・目次メンテナンス・出力ファイル・命名規則・タイムスタンプを扱う各テストファイルを案内します。
- 実装変更の影響範囲を、どのテストを読むべきかという観点で素早く切り分けるための目次です。

## Read this when

- `tests` 配下にどんな回帰テストがあるか全体像を把握したいとき。
- `INDEX.md` メンテナンス、repo 共通処理、CLI 入口、タイムスタンプなどのテスト範囲を確認したいとき。
- 特定のテストファイルが何を守っているかを素早く見分けたいとき。

## Do not read this when

- `src` 配下の実装ロジックやサブコマンド仕様そのものだけを確認したいとき。
- `oracles` 側の正本仕様やルーティング方針だけを確認したいとき。
- 個別の pytest ヘルパーや fixture の詳細だけを探したいとき。

## hash

- 2d593caab537fe25115f2213e4cb09892534dab89d17803ad7ba747579675338
