...
- `src` は cmoc の Python 実装本体の入口で、CLI エントリーポイント、共通処理、サブコマンド実装をまとめています。
- `src/main.py` は `cmoc` CLI の起動口で、Typer アプリと各サブコマンドを登録します。
- `src/commons` と `src/sub_commands` はそれぞれ共通処理とサブコマンド実装の入口で、配下の `INDEX.md` から個別モジュールへ進みます。

## Read this when

- `cmoc` の起動口やサブコマンド登録の全体像を追いたいとき。
- 共通処理とサブコマンド実装の役割分担を確認したいとき。
- `src` 配下のどの入口から辿るべきかを判断したいとき。

## Do not read this when

- 個別サブコマンドの利用手順だけを見たいときは、`oracles/app_specs/sub_commands/` を読むべきです。
- `src` 配下の具体的な実装詳細ではなく、共通ルールや設計方針だけを確認したいときは、`oracles/dev_rules/` を読むべきです。
- テストや評価レポートなど、`src` 以外の領域を探したいときはこの目次は不要です。

## hash

- bc1db9479e090f6c6a5302aff6fe6390eca0f43260061c7314059cd1a02eba8a
...
- `tests` 配下の pytest 共通設定と各機能別テスト群の入口です。
- `conftest.py` で import path を調整し、`test_codex.py` で Codex CLI 呼び出し周辺を検証します。
- `test_indexing.py`、`test_repo.py`、`test_subcommands.py` で INDEX メンテナンス、git 共通処理、サブコマンド制御を扱います。
- `test_file_naming.py` と `test_timestamps.py` で命名規則や時刻・経過時間の仕様を確認します。

## Read this when

- `tests` 配下のどのテストファイルが何を担当するか把握したいとき。
- pytest の共通設定や import path の変更箇所を確認したいとき。
- Codex CLI 連携、`INDEX.md` メンテナンス、git 共通処理、サブコマンド回帰テストの入口を探したいとき。
- タイムスタンプやファイル命名の仕様を確認したいとき。

## Do not read this when

- 本番実装の詳細を追いたいときは `src` 側を読むべきです。
- `oracles` の正本仕様やルーティング方針だけを確認したいときは、このディレクトリではなく `oracles` 側を読むべきです。
- pytest 以外のテスト基盤や CI 設定を確認したいときは、このディレクトリだけでは足りません。
- 個別のテストケース本体だけを見たいときは、該当する `test_*.py` を直接読むべきです。

## hash

- 1051a363e05475ab958b2ca9e81849b0a95c397e3c5d4394080bfb882926051f
