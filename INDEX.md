...
- `src` は cmoc の実装コードをまとめるルートディレクトリの目次です。
- この配下では `main.py` が CLI エントリーポイント、`commons` が共通基盤、`sub_commands` が各サブコマンド本体を担当します。
- `src` 全体の構成を把握して、どの実装ファイルや下位ディレクトリを読むべきか切り分けるための入口です。

## Read this when

- cmoc の実装コード全体の配置を把握したいとき。
- CLI エントリーポイント、共通基盤、サブコマンド本体のどこを読むべきか判断したいとき。
- `src/main.py`、`src/commons`、`src/sub_commands` の役割分担を確認したいとき。

## Do not read this when

- 個別モジュールの実装詳細だけを確認したいとき。
- `oracles` 側の正本仕様やユーザー向け実行時仕様だけを確認したいとき。
- cmoc 自体の開発ルール、テスト規約、運用ルールだけを調べたいとき。

## hash

- 5b1787831ed2b432bae6fad80669d244c25e77ac4f54d335a912d2a39c5b4f41
...
- `tests` 配下の pytest テスト群を案内するルーティング用目次です。共通設定 `conftest.py` と、Codex 呼び出し、ファイル命名、`INDEX.md` メンテナンス、git リポジトリ共通処理、サブコマンド制御、タイムスタンプ仕様の各テストへの入口をまとめます。
- `commons.codex` の呼び出しラッパー、`commons.indexing` の `INDEX.md` 更新、`commons.repo` の列挙・差分判定、`sub_commands` の決定論的な制御ロジック、日時と経過時間表示の回帰テストを横断して案内します。
- `tests/INDEX.md` 配下の個別ファイルを読む前に、どの機能のテストがどこにあるかを把握するための入口です。

## Read this when

- `tests` 配下のどのファイルがどの機能を検証しているかを把握したいとき。
- pytest の共通設定 `conftest.py` が何をしているか確認したいとき。
- Codex CLI 呼び出し、Structured Output、ログ、リトライのテストを探したいとき。
- `INDEX.md` の自動生成・更新や gitignore 除外の検証を探したいとき。
- git リポジトリの探索・差分判定・削除検出のテストを探したいとき。
- サブコマンドの実行フロー、エラー処理、終了コード、出力の tee を確認したいとき。
- タイムスタンプや経過時間表示のフォーマット検証を探したいとき。

## Do not read this when

- 個別テストの実装内容だけを直接追いたいとき。
- cmoc のユーザー向けコマンド仕様や oracle 正本仕様だけを知りたいとき。
- `src` 配下の本体実装や設計方針だけを調べたいとき。
- `README.md`、`AGENTS.md`、`oracles`、`memo` の運用ルールだけを確認したいとき。
- `tests` 全体の入口ではなく、特定の一つのテストファイルがすでに決まっているとき。

## hash

- 8b57c8fdf47136e96e3caeef69d56a5d4651ea50c2033dac6a5f1014e05105d2
