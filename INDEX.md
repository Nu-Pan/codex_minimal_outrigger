...
- cmoc の実装コード全体の入口で、`main.py`、`commons/`、`sub_commands/` の役割を案内する目次です。
- CLI エントリーポイント、共通ユーティリティ、サブコマンド実装へ進むためのルーティング層です。
- 実装本体や個別機能の詳細に入る前に、どのファイルを読むべきかを切り分けます。

## Read this when

- cmoc の実装ソース全体の配置と役割分担を俯瞰したいとき。
- `src/main.py` が担当する CLI 起動とコマンド登録の入口を探したいとき。
- `src/commons` の共通処理と `src/sub_commands` の各サブコマンド実装へどの順で進むか整理したいとき。
- 実装コードを読み始める前に、まずどのディレクトリを参照すべきか決めたいとき。

## Do not read this when

- 個別サブコマンドの引数、状態遷移、終了条件などの詳細仕様だけを確認したいとき。
- `commons` 内の共通ユーティリティや `sub_commands` 内の個別実装だけを直接確認したいとき。
- `oracles` 配下の正本仕様や使用手順だけを確認したいとき。
- テストコードや `INDEX.md` 生成ロジックそのものの詳細だけを追いたいとき。

## hash

- 84941aaf68a2f46d9632b9c7015a96aa9945e264a3919336f8d883275f099dba
...
- pytest ベースの cmoc テスト群の入口で、`src` 実装の回帰確認をまとめます。
- `conftest.py` の共有設定と、Codex・INDEX.md・repo・subcommand・timestamp・命名規則の各テストへ案内します。
- 個別テストの役割分担をたどり、どの仕様変更がどのテストに影響するか見分けるための目次です。

## Read this when

- pytest 全体のテスト入口や、どのテストファイルを読むべきか確認したいとき。
- `commons.codex`、`commons.indexing`、`commons.repo`、サブコマンド群の変更がどの回帰テストに触るか整理したいとき。
- 新しいテストを追加・既存テストを修正する前に、既存のテスト群の配置を把握したいとき。
- テスト共通設定やフィクスチャ、命名規則、タイムスタンプ形式の検証箇所を探したいとき。

## Do not read this when

- 本体実装や `oracles` の正本仕様だけを確認したいとき。
- 個別の関数実装や CLI 挙動を追いたいときは、対応する `src` 側か `oracles` 側を直接読むべきです。
- README、AGENTS、memo の運用ルールだけを確認したいとき。
- このディレクトリのテストを実行せず、仕様文書だけを確認したいとき。

## hash

- 1cc3304425ed7a5f2973553f0d00bd34f02f801838e659bbf0a84a4cf8d68735
