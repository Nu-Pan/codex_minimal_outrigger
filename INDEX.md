...
- `src` は cmoc の Python 実装を置く最上位のソースツリーです。
- CLI エントリーポイントの `main.py` と、共通処理の `commons`、サブコマンド実装の `sub_commands` で構成されています。
- 個別機能の詳細へ進む前に、この配下の役割分担と入口を確認するための目次です。

## Read this when

- `cmoc` の起動入口やサブコマンド登録の全体像を把握したいとき。
- 共通ユーティリティとサブコマンド実装のどちらを読むべきか判断したいとき。
- `src` 配下のどのディレクトリから辿るべきか整理したいとき。

## Do not read this when

- `main.py` や `commons`、`sub_commands` の個別実装を直接見たいとき。
- `cmoc` の利用手順や `oracles` の仕様断片だけを確認したいとき。
- この目次ではなく、目的のモジュール本文や下位 `INDEX.md` を読むべきとき。

## hash

- d5514acd1873d7962be62c7343e9f29b291d3356928f2cfeb79e5f653693ff39
...
- 共通設定の `conftest.py` と、`codex` 実行・`repo` 共通処理・`INDEX.md` 生成・サブコマンド状態遷移・タイムスタンプ表示を検証するテストを案内します。
- 個別テストの詳細は各 `test_*.py` を直接参照する前提の目次です。

## Read this when

- `tests` 配下にどのテストがあり、何を検証しているかを俯瞰したいとき
- `conftest.py` の共通設定、`codex` 呼び出し、`repo`、`indexing`、`subcommands`、`timestamps` の各テスト入口を探したいとき
- どのテストファイルがどの責務を持つかを素早く把握したいとき

## Do not read this when

- cmoc の個別サブコマンド仕様や実行フローだけを確認したいとき
- `src` 側の実装ロジックそのものを追いたいとき
- `oracles` の正本仕様や `INDEX.md` 生成ルールだけを確認したいとき

## hash

- 7097bc6af89b3b7cd0644d9e1f198d4000a693f045dc2914588cc3f93115690d
