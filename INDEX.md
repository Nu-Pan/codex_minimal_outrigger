...
- `cmoc` の実装本体を置く `src` 直下の入口で、`main.py`、`commons/`、`sub_commands/` をまとめます。
- CLI の起動点、共通基盤、各サブコマンド実装の配置先を切り分けて把握するための目次です。
- `src` 配下の各モジュールを読む前に、役割の境界を確認するために使います。

## Read this when

- `cmoc` の起動点がどこか、`src/main.py` が何をするか確認したいとき。
- 共通ユーティリティ群が `src/commons/` にあり、どのような役割を持つか把握したいとき。
- `src/sub_commands/` にある各サブコマンド実装の配置先を整理したいとき。
- `src` 配下の各モジュールを読む前に、役割の境界を素早く確認したいとき。

## Do not read this when

- `src` 配下の個別サブコマンド実装や共通モジュールの詳細だけを確認したいときは、該当ディレクトリの `INDEX.md` を直接読むべきです。
- `oracles` の正本仕様や利用手順だけを確認したいときは、この入口ではなく `oracles/` 側を読むべきです。
- テストの期待値や実装細部だけを追いたいときは、`tests/` や対象モジュールを直接見るべきです。

## hash

- fc8c9b867783b3e195dc553a53bdb80f22d8ea9542a88fdb6297cb8f64d9ccc9
<!-- cmoc-index-kind: directory -->
...
- pytest の共通設定と cmoc 本体の回帰テストをまとめたディレクトリの入口です。
- Codex 呼び出し、`INDEX.md` の維持、Git リポジトリ操作、サブコマンド制御、レポート保存、タイムスタンプ関連のテストが入っています。
- 個別のテストファイルへ進む前に、テスト対象の機能群を素早く見分けるための目次です。

## Read this when

- pytest の共通設定や `src` の import path 追加方法を確認したいとき。
- `codex` 呼び出し、`INDEX.md` メンテナンス、repo 判定、レポート保存、タイムスタンプなどの回帰テストを探したいとき。
- `tests` 配下で、どの機能がどのテストファイルにまとまっているかを把握したいとき。

## Do not read this when

- cmoc の実装本体や共通モジュールのコードを確認したいとき。
- `oracles` 配下の正本仕様や運用手順だけを確認したいとき。
- 個別テストの細かな期待値やアサーションだけを追いたいとき。

## hash

- 6a4d92bae88ac05d1b1a4439b567859af0ff52473c59d3606834b5676b854d72
<!-- cmoc-index-kind: directory -->
