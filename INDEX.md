...
- `main.py`、`commons`、`sub_commands` のうち特定の 1 つだけを見たいときは、該当ファイルやサブディレクトリを直接読むべきです。
- サブコマンド仕様や設計ルールだけを確認したいときは、この入口ではなく `oracles` 側を読むべきです。
- Python パッケージの配布用メタデータやキャッシュだけを見たいときは、この目次ではなく対象の生成物を直接確認すべきです。
