...
- `src` 配下に、`cmoc` の実装本体をまとめたコード群が入っています。
- CLI の起動入口、共通基盤処理、個別サブコマンド実装の 3 つを中心に構成されています。
- このディレクトリは、`cmoc` の実装を追うための最上位の入口です。

## Read this when

- `cmoc` の CLI エントリーポイントやサブコマンド登録の全体像を確認したいとき。
- 共通基盤モジュールとサブコマンド実装の役割分担を俯瞰したいとき。
- `src/main.py`、`src/commons`、`src/sub_commands` のどこに何があるかを整理したいとき。
- 実装を始める前に、`cmoc` の主要なコード構成をざっと把握したいとき。

## Do not read this when

- `cmoc` の個別サブコマンド 1 つだけの詳細仕様を確認したいときは、このディレクトリ全体ではなく該当モジュールを直接読むべきです。
- `oracles` 配下の正本仕様そのものを確認したいときは、このディレクトリではなく `oracles` 側を読むべきです。
- 実装ではなくテストコードだけを探しているときは、このディレクトリではなく `tests` を読むべきです。
- CLI の設計方針や共通ルールだけを確認したいときは、`src` 全体ではなく該当する共通仕様や開発ルールを読むべきです。

## hash

- e476efd083ebe2aa92a5bef65513717d9ef00847192ab3222b40f8a6b9ef9ecf
...
- `tests` 配下の pytest 群の入口で、cmoc の決定論的な制御ロジックをまとめて検証します。
- `test_codex.py`・`test_indexing.py`・`test_repo.py`・`test_subcommands.py`・`test_timestamps.py` が主要機能の回帰を担います。
- `conftest.py` は共通の import path 設定を提供し、各テストが `src` 配下を直接 import できるようにします。

## Read this when

- テスト全体の役割分担を把握したいとき。
- どの機能変更がどのテストに影響するかを先に確認したいとき。
- 新しいテストや共通 fixture を追加する前に、既存の入口を確認したいとき。

## Do not read this when

- 個別の本体実装や CLI 仕様そのものを確認したいとき。
- `oracles` の正本仕様やリポジトリ運用ルールだけを確認したいとき。
- テストの配置方針ではなく、実行環境や依存関係の設定だけを確認したいとき。

## hash

- 6233b80bc4ce2f1516911a3c85c7ec86442e7998e8f4bbb5c9f9583b89c84525
