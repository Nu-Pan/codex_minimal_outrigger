...
- `src` は cmoc の実装ルートで、CLI エントリーポイントと共通処理、サブコマンド実装をまとめる入口です。
- `main.py` は `cmoc` CLI の起点で、`init`、`session`、`apply`、`review` のコマンド登録と例外整形を扱います。
- `commons/` は全体で共有する基盤モジュール群、`sub_commands/` は各サブコマンド本体の入口です。

## Read this when

- cmoc のソース全体で、起点・共通基盤・サブコマンド実装の配置を俯瞰したいとき。
- `cmoc` の起動経路やコマンド登録がどこにあるか確認したいとき。
- 共通処理を探したいときに `commons/`、各コマンド本体を探したいときに `sub_commands/` へ進みたいとき。

## Do not read this when

- 個別サブコマンドの詳細仕様や引数だけを確認したいときは、この入口ではなく該当モジュールや `oracles` を読むべきです。
- 共通モジュールの内部実装だけを追いたいときは、`commons/` 配下の個別ファイルを直接読むべきです。
- サブコマンド本体の業務ロジックだけを追いたいときは、`sub_commands/` 配下の個別モジュールを直接読むべきです。

## hash

- 22d55a1f49e17eb39c42b51bd61fac2b5ffeb2d2c47c7957cf16e9880e4059a3
...
- 788035b31b82872a8ddf554849466a99ee3bcb8f612419ecb896a8eb9364318d
