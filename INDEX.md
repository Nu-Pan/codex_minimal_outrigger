...
- `src` は cmoc の実装コードのルートで、CLI エントリーポイントと共通基盤、サブコマンド実装への入口をまとめる場所です。
- 直下には `main.py`、`commons/`、`sub_commands/` があり、実行開始点と共通処理・コマンド本体を分担しています。
- 各配下の詳細は、それぞれの `INDEX.md` を参照します。

## Read this when

- `src` 直下にどんな実装入口があるかを一覧で確認したいとき。
- CLI 起動点、共通モジュール、サブコマンド実装の担当範囲を整理したいとき。
- `src/main.py`、`src/commons`、`src/sub_commands` のどれを先に読むべきか判断したいとき。

## Do not read this when

- `src/commons` の共通処理だけを深く見たいときは、`src/commons/INDEX.md` を読むべきです。
- `src/sub_commands` の個別コマンド実装だけを追いたいときは、`src/sub_commands/INDEX.md` を読むべきです。
- `cmoc` の利用手順や `oracles` の正本仕様だけを確認したいときは、この目次ではなく該当する仕様文書を読むべきです。

## hash

- 088325226d79fbf98c4e6a9724ba104b0f5ebc1dc060e2e7b8188eef976c37cc
...
- pytest による cmoc の回帰テスト群をまとめた入口です。
- `src` の import 設定、Codex 呼び出し、INDEX.md 生成、git リポジトリ共通処理、CLI サブコマンド、タイムスタンプ仕様を検証します。
- 各テストファイルは機能別の回帰点を担い、`tests/INDEX.md` はその案内役です。

## Read this when

- テスト全体の役割や、どの機能がどのテストファイルで守られているかを確認したいとき。
- `commons.indexing`、`commons.repo`、`commons.codex`、`sub_commands` 周辺を変更して、影響範囲を見たいとき。
- pytest 共通設定や個別テストの入口を探したいとき。

## Do not read this when

- cmoc の利用手順や `oracles` の正本仕様だけを確認したいとき。
- 実装コードそのものを読みたいときは `src` 配下を直接見るべきです。
- 特定のテストケースの詳細だけを追うときは、このディレクトリの INDEX ではなく該当する `test_*.py` を直接読むべきです。

## hash

- 04df1445e17e9c3930568d9ee0bdc115e868bdd3fedbe48441979d72ff5399cd
