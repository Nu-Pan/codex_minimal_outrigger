...
- `src` は cmoc の実装本体をまとめたソースコードの入口です。
- `main.py` に CLI の起動口があり、`commons/` に共通処理、`sub_commands/` に各サブコマンド実装が置かれています。
- 実装全体の配置と責務分担を把握するための目次です。

## Read this when

- cmoc の実装がどのファイルとパッケージに分かれているかを確認したいとき。
- CLI 起動口、共通処理、サブコマンド実装の責務分担を俯瞰したいとき。
- `src` 配下のどこから読み始めればよいかを判断したいとき。

## Do not read this when

- 個別モジュールの実装だけを確認したいときは、`main.py`、`commons/`、`sub_commands/` を直接読むべきです。
- cmoc の仕様断片や利用手順だけを確認したいときは、`oracles/` 側を読むべきです。
- テスト実装だけを確認したいときは、`tests/` を読むべきです。

## hash

- 54c0fc346143ee4bc3d0f4293349479dec3c20fe5d01fe52a9472db25def792c
...
- `tests` は cmoc の回帰テスト群の入口で、共通処理・CLI 制御・日時処理などのテストをまとめて案内します。
- `conftest.py` は pytest の import path 設定を担い、各 `test_*.py` は機能別に責務を分けて配置されています。
- この目次は、どのテストファイルに進むべきかを判断するための案内です。

## Read this when

- `tests` 配下にどの回帰テストがあり、何を検証しているかを俯瞰したいとき。
- `codex` 呼び出し、`INDEX.md` メンテナンス、リポジトリ操作、サブコマンド制御、日時・計時の各テスト群の入口を確認したいとき。
- pytest の共通設定や、テスト群ごとの役割分担を素早く把握したいとき。
- 新しいテストを追加する前に、既存のテスト配置と責務境界を確認したいとき。

## Do not read this when

- `src/` 側の実装ロジックや関数本体だけを確認したいとき。
- `oracles` 側の正本仕様やサブコマンド仕様の詳細だけを確認したいとき。
- `README.md`、`AGENTS.md`、`memo` の運用や編集可否だけを確認したいとき。
- 個別のテストケースを直接読みたいときは、`test_codex.py` や `test_repo.py` など該当ファイルを読むべきです。

## hash

- 3175f8e1b001718113fff91354a4ba7f68fc68648d08a5f01d847d63458dc354
