...
- 7cf49ffa3cf6c1e5cb64fc1c0ad3fb581866f1048da7508132d72f5bc32346e7
...
- `tests` 配下の回帰テスト群をまとめた入口です。
- pytest の共通設定、Codex CLI 呼び出し、git 共通処理、レポート保存、サブコマンド制御、タイムスタンプ仕様を扱うテストを案内します。
- 個別テストファイルの詳細へ進む前に、テスト群全体の役割を把握するための目次です。

## Read this when

- `tests` ディレクトリ全体が何を検証しているかを俯瞰したいとき。
- `cmoc` の回帰テストがどの機能群を守っているかを素早く把握したいとき。
- 新しいテストを追加・修正する前に、既存テストの責務分担を確認したいとき。

## Do not read this when

- 個別の `test_*.py` のテストケース内容や期待値を追いたいとき。
- `src` 配下の実装ロジックや CLI 本体の仕様だけを確認したいとき。
- `INDEX.md` の生成ルールや `oracles` 側の正本仕様だけを確認したいとき。

## hash

- 753045785aacc2cf08a626983d014c9e7dcd321c2abf6e5fdd87e2b0cc24ca57
