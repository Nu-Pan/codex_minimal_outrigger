...
- `src` は cmoc の実装本体を置くソースディレクトリで、CLI エントリーポイントと共通基盤、サブコマンド実装をまとめます。
- `main.py` は CLI の起点、`commons/` は横断的な共通処理、`sub_commands/` は各コマンド本体への入口です。
- この目次は、`src` 配下を役割ベースで素早く振り分けるためのルーティング文書です。

## Read this when

- `src` 配下にどの役割のモジュールがあるかを全体俯瞰したいとき。
- CLI エントリーポイント、共通基盤、サブコマンド実装のどこに進むべきか振り分けたいとき。
- `src` ディレクトリの案内文書として、下位の `INDEX.md` へ進む前の入口を確認したいとき。

## Do not read this when

- `src/main.py` や個別サブコマンドの実装だけを直接確認したいとき。
- `src/commons` や `src/sub_commands` の中でも、読む対象ファイルがすでに決まっているとき。
- `src` 配下の自動生成物やビルド成果物ではなく、別ディレクトリの仕様を調べたいとき。

## hash

- 4dd37e0a61d9e446755c026103e25af89a37ef3ebfab5fd56bda8fce8223a09d
...
- cmoc の回帰テストをまとめたディレクトリの入口です。
- `src` の実装に対する検証を、機能別のテストファイルへ分けて管理しています。
- `INDEX.md` のメンテナンス、repo 判定、ファイル命名、レポート保存、タイムスタンプ、CLI 挙動をまとめて確認するための目次です。

## Read this when

- `INDEX.md` 保守、リポジトリ判定、ファイル命名、レポート保存、タイムスタンプ、CLI サブコマンドなど、cmoc の回帰テストを探したいとき。
- どのテストファイルがどの挙動を守っているかを素早く把握したいとき。
- テスト追加や修正の前に、このディレクトリにある回帰観点の全体像を確認したいとき。

## Do not read this when

- cmoc の実装本体や `src` 配下のロジックを確認したいとき。
- `oracles` 配下の正本仕様や仕様断片だけを追いたいとき。
- pytest の共通設定やテスト補助の詳細ではなく、個別の回帰観点だけを確認したいとき。

## hash

- 15c3cd41af4efc76427e5ca242f0d71fe830d2644ec7763a448c2134e568b240
