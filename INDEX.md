...
- `cmoc` の実装コード全体の入口で、`commons`、`main.py`、`sub_commands` をまとめます。
- `main.py` は CLI の起動点、`commons` は共通処理、`sub_commands` は個別サブコマンド実装への入口です。
- それぞれの詳細は下位の `INDEX.md` や個別モジュールへたどります。

## Read this when

- `src` 配下の役割分担を把握したいとき。
- CLI 起動経路とサブコマンド登録の入口を確認したいとき。
- 共通処理の所在や、どの実装モジュールを読むべきかを切り分けたいとき。
- `main.py`、`commons`、`sub_commands` のどこへ進むべきか整理したいとき。

## Do not read this when

- 特定の機能の実装詳細だけを確認したいときは、対応するモジュールを直接読むべきです。
- `commons` や `sub_commands` の下位構造だけを確認したいときは、それぞれの `INDEX.md` を読むべきです。
- CLI の利用手順や仕様断片だけを確認したいときは、`oracles` 側を読むべきです。
- テストや設計ルールだけを追いたいときは、`tests` や `oracles/dev_rules` を参照すべきです。
...
- `tests` 配下の pytest テスト群の入口で、`conftest.py` と各 `test_*.py` への案内をまとめます。
- 共通 import 設定、Codex CLI ラッパー、`INDEX.md` メンテナンス、git 共通処理、サブコマンド制御、タイムスタンプ仕様を扱います。
- この配下には個別テストの実装があり、`tests/INDEX.md` は該当ファイルへ進むための目次です。

## Read this when

- pytest 用の共通設定や個別テスト群の役割分担を確認したいとき。
- Codex CLI 呼び出し、`INDEX.md` メンテナンス、git 共通処理、サブコマンド、タイムスタンプのどれを読むべきか整理したいとき。
- `tests` 配下の各ファイルの担当範囲を把握したいとき。

## Do not read this when

- 実装コードや仕様本文そのものを確認したいときは、`src/` や `oracles/` 側を直接読むべきです。
- `README.md`、`AGENTS.md`、`memo` の運用や編集可否だけを確認したいとき。
- 特定の 1 テストケースの詳細だけを知りたいときは、該当する `test_*.py` を直接読むべきです。

## hash

- 94f995c616cd5c67225a047111b4b7a14dccc249a0543e49b7eacda4983a80e7
