    ...
    *,
    ...
    for index, oracle_file in enumerate(oracle_files, start=1):
        ...
        output = run_codex_exec(
            ...
            read_only=True,
            expect_json=False,
            text_validator=_validate_evaluation_output,
        )
...
def _validate_evaluation_output(output: str) -> None:
    """評価レポートの必須明示事項を機械的に確認する。"""
    # 必須見出しを欠く自然言語出力はレポートとして受け取らない。
    missing = [
        heading
        for heading in _REQUIRED_EVALUATION_HEADINGS
        if heading not in output
    ]
    if missing:
        raise ValueError(
            "Evaluation report missed required headings:\n"
            + "\n".join(missing)
        )
