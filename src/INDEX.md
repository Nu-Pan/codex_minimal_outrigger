...
- cmoc 全体で使う共通処理をまとめた Python パッケージです。
- Codex CLI 呼び出し、サブコマンド実行制御、共通エラー、リポジトリ探索、`INDEX.md` 生成、ログ、タイムスタンプ、経過時間計測などの横断的な処理を含みます。
- 各モジュールは個別サブコマンドではなく、`src` 配下の実装から再利用する前提です。

## Read this when

- 共有ユーティリティや共通制御をどこに置くか確認したいとき。
- `codex exec` の共通ラッパー、`INDEX.md` の自動生成、git と作業ツリーの共通処理を見たいとき。
- サブコマンド共通のログ、タイミング計測、エラー整形、タイムスタンプ生成を確認したいとき。

## Do not read this when

- 個別サブコマンドの引数や業務ロジックだけを追いたいとき。
- CLI 全体の使用手順やワークフローだけを確認したいとき。
- `src/commons` 以外の実装やテストを探しているとき。

## hash

- beb962b3d61e43e8e44e43c4810b7db158efa15aa0eb3dcb05685aa351c5d870
