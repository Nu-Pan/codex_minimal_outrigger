...
- `cmoc` で各サブコマンドから共通利用する補助モジュール群をまとめたディレクトリです。
- `codex` 呼び出し、`repo` 探索と `.cmoc` 管理、共通エラー整形、サブコマンド実行ラッパー、ログ、タイムスタンプ、経過時間計測、`INDEX.md` メンテナンスを扱います。
- 個別サブコマンドの業務ロジックではなく、横断的な基盤処理を収めています。

## Read this when

- サブコマンドから使う共通処理の置き場所や責務分担を確認したいとき。
- `codex exec` 呼び出し、`INDEX.md` 自動生成、`git` ルート探索、session state、ログ、エラー表示、時間計測の実装や修正をしたいとき。
- 共有モジュール間の依存関係や、`src/main.py` や `src/sub_commands` からの呼び出し経路を追いたいとき。
- 共通ユーティリティのテストやリファクタリング方針を整理したいとき。

## Do not read this when

- 個別サブコマンドの引数、状態遷移、ユーザー向け手順だけを確認したいときは、`src/sub_commands` 側を読むべきです。
- `oracles` の正本仕様や `INDEX.md` の生成ルールだけを確認したいときは、このディレクトリではなく仕様側を読むべきです。
- 実装コードの細部ではなく、リポジトリ運用や README のような文書だけが目的のときは、このディレクトリを読む必要はありません。
- `src/commons` のどれか 1 ファイルだけを追いたいときは、該当モジュールを直接読むべきです。

## hash

- 2900c0f84133c4f36df0397864223d5b1061e9c981d5742be8606f701530c773
