...
- cmoc 全体で再利用する共通処理を集めたディレクトリです。
- `codex exec` 呼び出しの共通ラッパー、サブコマンド実行制御、共通エラー整形を含みます。
- リポジトリ探索、INDEX.md 更新、サブコマンドログ、タイムスタンプ、経過時間計測のユーティリティをまとめています。
- `__init__.py` はパッケージ定義のみを担い、実行ロジックは持ちません。

## Read this when

- サブコマンド間で共有する処理や定数、例外、ログ、計測の置き場所を確認したいとき。
- `codex exec` の共通実行、Structured Output 検証、再試行や quota 復帰の流れを追いたいとき。
- CLI の共通実行ラッパー、`typer.Exit` の扱い、サブコマンド終了時の集計出力を確認したいとき。
- リポジトリ root 探索、`.cmoc` 周辺の判定、共有ログや `INDEX.md` の更新仕様を確認したいとき。
- 経過時間表示やタイムスタンプ生成のフォーマットを確認したいとき。

## Do not read this when

- 個別サブコマンドの業務ロジックだけを確認したいときは、`src/sub_commands` 側を直接読むべきです。
- サブコマンドごとの CLI 引数やユーザー向けコマンド仕様だけを確認したいときは、このディレクトリを読む必要はありません。
- コーディング規約、設計規約、テスト規約そのものを確認したいときは、`oracles/dev_rules` を読むべきです。
- `cmoc` の仕様断片やルーティング方針だけを確認したいときは、`oracles/app_specs` 側を優先すべきです。
- `tests` の期待値や Fake Codex CLI の使い方だけを確認したいときは、このディレクトリではなく `tests` やテスト仕様を参照すべきです。

## hash

- 61530b8fad1693ad5ce94b905e306a67e2cbfc6ad620e21cf108c7ba1b492317
