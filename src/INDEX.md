...
- `src/commons` 配下にある共有モジュール群の入口で、`codex` 実行ラッパー、サブコマンド共通実行制御、共通エラー、リポジトリ判定、`INDEX.md` 生成、サブコマンドログ、タイムスタンプ、経過時間計測をまとめています。
- 各モジュールは CLI 本体や個別サブコマンドから再利用される基盤処理を提供し、`repo` / `errors` / `timing` / `subcommand_log` などの役割分担を見分けるための目次です。
...
- 38dde35654163371ed12644804f7c32cf1e28c46e976413944151ae7da185bf7
