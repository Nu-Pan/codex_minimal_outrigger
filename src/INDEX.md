...
- cmoc 全体で再利用する共通処理を集めたディレクトリの入口です。
- `codex exec` の共通ラッパー、サブコマンド実行制御、共通エラー整形を含みます。
- リポジトリ探索、`INDEX.md` 更新、サブコマンドログ、タイムスタンプ、経過時間計測のユーティリティを案内します。
- `__init__.py` はパッケージ宣言のみを担い、実行ロジックは持ちません。

## Read this when

- サブコマンド間で共有する処理、定数、例外、ログ、計測の置き場所を確認したいとき。
- `codex exec` の共通実行、Structured Output 検証、再試行、quota 待機の流れを追いたいとき。
- `git` リポジトリ探索、`.cmoc` の追跡除外、`INDEX.md` の維持、サブコマンドログの保存仕様を確認したいとき。
- 経過時間表示やタイムスタンプ生成のフォーマットを確認したいとき。

## Do not read this when

- 個別サブコマンドの実装や `src/sub_commands` 側の業務ロジックだけを確認したいとき。
- `oracles` 側の正本仕様や、`INDEX.md` の生成ルールそのものだけを確認したいとき。
- `tests` の期待値や Fake Codex CLI の使い方だけを確認したいとき。
- `README.md`、`AGENTS.md`、`memo` の運用ルールや編集可否だけを確認したいとき。

## hash

- 333ecb55c5999c7095d7b3b9d0acc9efb71b92571d4da242cdca6088ffdd0885
