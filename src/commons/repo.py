...
def ensure_cmoc_ignored(repo_root: Path) -> bool:
    """`.cmoc` が git 追跡対象外になるよう `.gitignore` を更新する。"""
    gitignore = repo_root / ".gitignore"
    existing = gitignore.read_text(encoding="utf-8") if gitignore.exists() else ""
    lines = existing.splitlines()
    if any(line.strip() in {".cmoc", ".cmoc/", "/.cmoc", "/.cmoc/"} for line in lines):
        return False

    # 既存内容の末尾改行を保ちながら ignore ルールを追記する。
    prefix = existing
...
def assert_only_oracles_uncommitted(repo_root: Path) -> None:
    ...
    outside = [path for path in changed_paths(repo_root) if not path.startswith("oracles/")]
    if outside:
        raise CmocError(
...
def changed_oracle_files(repo_root: Path, base_commit: str) -> list[Path]:
    ...
    # base..HEAD の変更ファイルを取り込む。
    committed = run_git(
        repo_root,
        ["diff", "--name-only", "--diff-filter=ACMRT", f"{base_commit}..HEAD", "--", "oracles"],
    )
    for line in committed.stdout.splitlines():
        collected.add(repo_root / line)

    # working tree と staging area の未コミット変更を取り込む。
    uncommitted = run_git(
        repo_root,
        ["diff", "--name-only", "--diff-filter=ACMRT", "HEAD", "--", "oracles"],
    )
    staged = run_git(
        repo_root,
        ["diff", "--cached", "--name-only", "--diff-filter=ACMRT", "--", "oracles"],
    )
    for output in [uncommitted.stdout, staged.stdout]:
        for line in output.splitlines():
            collected.add(repo_root / line)

    # 未追跡ファイルも working tree 上の未コミット変更として扱う。
...
def _is_gitignored(repo_root: Path, relative_path: str) -> bool:
    ...
    result = run_git(repo_root, ["check-ignore", "-q", "--", relative_path], check=False)
    if result.returncode == 0:
        return True
    if result.returncode == 1:
        return False

    # git 管理外の一時テスト用 fallback として単純な glob を読む。
    gitignore = repo_root / ".gitignore"
    ...
    for raw_line in gitignore.read_text(encoding="utf-8").splitlines():
        ...
        if not line or line.startswith("#"):
            continue
        pattern = line.lstrip("/")
        if fnmatch.fnmatch(relative_path, pattern) or relative_path.startswith(pattern.rstrip("/") + "/"):
            return True
    return False
